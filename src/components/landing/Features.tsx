...
export function Features() {
  return (
    <section id="features" className="scroll-mt-20 border-y border-olive/10 bg-vert-nuit/40 px-5 py-16 sm:py-24">
        ...
        <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {c.features.items.map((f, i) => (
            <motion.div
              ...
              className="relative rounded-2xl border border-olive/15 bg-white/[0.035] p-4"
            >
              {"live" in f && f.live ? (
                <span className="absolute right-3 top-3 rounded-full bg-terra/15 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-terra">
                  Bêta
                </span>
              ) : null}
